(async function() {
	...
	if (!document.getElementById("snowball_container")) {
			...
			return new Promise((resolve) => {
				browser.storage.local.get(['selectedLanguage'], (result) => {
					if (result.selectedLanguage) {
						...
						browser.storage.local.set({ selectedLanguage: defaultLang }, () => {
		...
		const realBrowser = isFirefox ? browser : (isbrowser ? browser : null);
		if (!realBrowser) {
		...
		function initializeLanguageSelector() {
			...
			if (languageSelect) {
				languageSelect.addEventListener('change', function() {
					...
					browser.storage.local.set({ selectedLanguage: selectedLang }, function() {
					location.reload();
					});
				});
			} else {
